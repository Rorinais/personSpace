...
<script setup>
import { blogPosts, getCategories } from '../lib/blog-posts.js'
import info from '../data/Information.json'
import projects from '../data/url-name.json'
import { ref } from 'vue'
    ...
    <div class="bento-content">
      ...
      <div class="right-main">
        <div class="card-grid">
          ...
          <div class="card card-person-info">
            <div class="card-inner bg-white">
              <div class="card-body">
                <h2>📋 个人资料</h2>
                <div class="info-grid">
                  <div><label>QQ群</label><span>{{ person.qqGroup }}</span></div>
                  <div><label>邮箱</label><span>{{ person.email }}</span></div>
                  <div><label>GitHub</label><a :href="person.github" target="_blank">{{ person.github?.split('/').pop() || 'Rorinais' }}</a></div>
                  <div><label>B站</label><a :href="person.bilibili" target="_blank">351668489</a></div>
                </div>
                <div class="info-desc">热爱编程与图形学，喜欢探索和分享技术。熟悉 Linux 环境，能独立完成网站开发与维护。</div>
              </div>
            </div>
          </div>
          ...
          <!-- 留言 -->
          <router-link to="/message" class="card card-message">
            <div class="card-inner bg-white">
              <div class="card-body">
                <h2>💬 留言板</h2>
                <div class="msg-preview">
                  <p>"网站很棒！期待更多内容" — 访客</p>
                  <p>"OpenGL 笔记很有帮助" — 访客</p>
                </div>
                <span class="card-btn">查看更多留言 →</span>
              </div>
            </div>
          </router-link>
...
.card-message { grid-column: span 4; }
.msg-preview { display: flex; flex-direction: column; gap: 4px; margin-bottom: 8px; }
.msg-preview p { font-size: 0.82rem; color: var(--text-muted); font-style: italic; }

.card-notes-link { grid-column: span 2; }
.card-notes-link .card-inner { min-height: 160px; }

.card-person-info { grid-column: span 2; }
.card-tech { grid-column: span 2; }
...
.tech-stack-item:hover { background: var(--accent); color: #fff; }
...
.info-grid span, .info-grid a { font-size: 0.88rem; color: var(--text-body); }
.info-grid a { color: var(--accent); }
.info-desc { font-size: 0.82rem; color: var(--text-muted); line-height: 1.7; margin-top: 14px; padding-top: 14px; border-top: 1px solid var(--border-color); }
...
@media screen and (max-width: 1024px) {
  ...
  .card-articles-wide, .card-message { grid-column: span 2; }
}
...
@media screen and (max-width: 480px) {
  ...
  .card-about, .card-projects-main, .card-notes-link, .card-person-info,
  .card-articles-wide, .card-message { grid-column: span 1; }
}
</style>
